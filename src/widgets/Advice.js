...
var Advice = {
    ...
    init: function (node) {
        Advice.state.dom = node;
        Advice.getData()
    },
    getData: function(){

       Advice.state.currentData = advice_data;
       //console.log(Advice.state.currentData)
       Advice.render();
    ...
    render: function(){
        var node = $(Advice.state.dom);

        let len = Advice.state.currentData.length;
        let minimum = 0;
        let maximum = len-1;
        let num = Math.floor(Math.random() * (maximum - minimum)) + minimum;
        let stringy = Advice.state.currentData[num];

        $(node).html("<h1>" + stringy + "</h1>");
        return stringy;
